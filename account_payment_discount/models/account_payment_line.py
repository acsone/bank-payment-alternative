...
class AccountPaymentLine(models.Model):
    ...
    @api.depends("order_id.date_prefered")
    def _compute_pay_with_discount(self):
        for rec in self:
            if not rec.can_have_discount:
                ...
                and rec.date_scheduled
            ):
                rec.pay_with_discount = rec.discount_date >= rec.date_scheduled
            else:
                rec.pay_with_discount = False
        ...
        if lines_with_discount:
            ...
            for aml in lines_with_discount.move_line_id:
                if aml.move_id._is_eligible_for_early_payment_discount(
                    ...
                    if refund_lines:
                        amt_curr = -refund_lines.amount_residual_currency
                        epd_aml_values_list.append(
                            {
                                "aml": refund_lines,
                                "amount_currency": amt_curr,
                                "balance": refund_lines.currency_id._convert(
                                    amt_curr,
                                    refund_lines.company_currency_id,
                                    date=payment_date,
                                ),
                            }
                        )
            ...
            open_amount_currency = diff * (-1 if self.payment_type == "outbound" else 1)
